...
type Kind = "avatar" | "cover" | "service";
...
export function ProfileMediaUpload({ userId, kind, path, demo, onUploaded }: Props) {
  ...
  const label = kind === "avatar" ? "Аватар" : kind === "cover" ? "Обложка" : "Фото услуги";
  ...
  return <div className="rounded-xl border border-dashed border-[var(--line)] p-4"><div className="flex items-center gap-3"><div className="grid h-12 w-12 place-items-center rounded-xl bg-[var(--soft)] font-bold text-[var(--accent)]">{kind === "avatar" ? "A" : kind === "cover" ? "▧" : "✦"}</div><div><p className="text-sm font-bold">{label}</p><p className="text-xs text-[var(--muted)]">JPEG, PNG или WebP · до 3 МБ</p></div></div>{path && <p className="mt-3 break-all text-xs text-[var(--muted)]">Файл загружен</p>}<input ref={input} type="file" accept="image/jpeg,image/png,image/webp" className="sr-only" onChange={(event) => upload(event.target.files?.[0])} /><button type="button" disabled={busy} onClick={() => input.current?.click()} className="focus-ring mt-4 rounded-full border border-[var(--line)] px-4 py-2 text-sm font-bold disabled:opacity-50">{busy ? "Загрузка…" : path ? "Заменить" : "Добавить фото"}</button>{message && <p role="status" className="mt-3 text-xs font-semibold text-[var(--brand)]">{message}</p>}</div>;
}
