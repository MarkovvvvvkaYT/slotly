...
export function BookingFlow({ services, availability = demoAvailability, profileId, bookings = demoBookings }: Props) {
  ...
  return <div className="surface mx-auto max-w-4xl overflow-hidden"><div className="border-b border-[var(--line)] px-6 py-6 sm:px-10"><div className="flex items-center justify-between"><div><p className="eyebrow">Записаться онлайн</p><h2 className="display mt-2 text-3xl font-bold sm:text-4xl">Найдите своё окно</h2></div><span className="text-sm font-bold text-[var(--muted)]">{step} <span className="font-normal">из</span> 3</span></div><div className="mt-6 h-1.5 overflow-hidden rounded-full bg-[var(--mint)]"><div className="h-full rounded-full bg-[var(--accent)] transition-all" style={{ width: `${(step / 3) * 100}%` }} /></div></div><div className="p-6 sm:p-10">
    ...
    {step === 3 && <div><p className="font-bold">Куда отправить подтверждение?</p>{customerEmail ? <p className="mt-3 rounded-xl bg-[var(--mint)] px-4 py-3 text-sm font-semibold text-[var(--brand)]">Вы вошли как {customerEmail}. Запись появится в <Link href="/account" className="underline">личном кабинете</Link>.</p> : <p className="mt-3 rounded-xl border border-[var(--line)] px-4 py-3 text-sm text-[var(--muted)]">Можно записаться как гость или <Link href="/login?next=/account" className="font-bold text-[var(--accent)] underline">войти как клиент</Link>, чтобы видеть запись в личном кабинете.</p>}<div className="mt-5 grid gap-4 sm:grid-cols-2"><label className="text-sm font-semibold">Имя<input required value={name} onChange={(event) => setName(event.target.value)} placeholder="Например, Анна" className="focus-ring mt-2 w-full rounded-xl border border-[var(--line)] bg-[var(--card)] px-4 py-3 font-normal outline-none transition focus:border-[var(--accent)]" /></label><label className="text-sm font-semibold">Телефон<input required value={phone} onChange={(event) => setPhone(event.target.value)} placeholder="+7 (___) ___-__-__" className="focus-ring mt-2 w-full rounded-xl border border-[var(--line)] bg-[var(--card)] px-4 py-3 font-normal outline-none transition focus:border-[var(--accent)]" /></label></div><label className="mt-4 block text-sm font-semibold">Комментарий <span className="font-normal text-[var(--muted)]">необязательно</span><textarea value={comment} onChange={(event) => setComment(event.target.value)} placeholder="Например, приду на макияж к фотосессии" rows={3} className="focus-ring mt-2 w-full resize-none rounded-xl border border-[var(--line)] bg-[var(--card)] px-4 py-3 font-normal outline-none transition focus:border-[var(--accent)]" /></label><div className="mt-6 rounded-2xl bg-[var(--mint)] p-4 text-sm"><div className="flex justify-between"><span className="text-[var(--muted)]">{service?.name}</span><strong>{service?.priceLabel}</strong></div><div className="mt-2 flex justify-between"><span className="text-[var(--muted)]">{new Date(`${selectedDate}T12:00:00`).toLocaleDateString("ru-RU", { day: "numeric", month: "long" })}</span><strong>{selectedTime}</strong></div></div></div>}
