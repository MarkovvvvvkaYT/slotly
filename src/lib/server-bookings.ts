...
export async function createServerBooking(input: BookingInput) {
  ...
  if (isSupabaseConfigured()) {
    ...
    const [{ data: profile }, { data: service }] = await Promise.all([
      ...
      supabase.from("services").select("id,name,profile_id,active,duration_minutes,deleted_at").eq("id", input.serviceId).eq("profile_id", input.profileId).eq("active", true).maybeSingle(),
    ]);
    if (!profile || !service || service.deleted_at) throw new Error("Услуга или профиль недоступны");
