...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await supabase.from("telegram_account_delete_challenges").select("id,status,expires_at,decided_at").eq("id", id).eq("user_id", userId).maybeSingle();
  if (error || !data) return NextResponse.json({ error: "Запрос не найден" }, { status: 404 });
