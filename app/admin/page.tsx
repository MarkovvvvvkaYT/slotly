...
export default async function AdminPage() {
  ...
  return <main className="specialist-theme min-h-screen bg-[var(--paper)]"><div className="mx-auto max-w-6xl px-5 py-5 lg:px-8"><AdminPageHeader profileName={data.profile.name} profileUrl={data.demo ? "/" : `/p/${data.profile.slug}`} /><AdminNavigation current="/admin" /></div><AdminDashboard initialBookings={data.bookings} initialServices={data.services} profile={data.profile} demo={data.demo} /></main>;
}
